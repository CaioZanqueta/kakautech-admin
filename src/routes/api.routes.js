...
router.post(
  ...
  async (req, res) => {
    try {
      ...
      if (format === "csv") {
        ...
        if (reportType === "tickets") {
          const csvData = (responsePayload.tickets || []).map((t) => ({
            ...
            "Data de Criação": new Date(t.createdAt).toLocaleDateString(
              "pt-BR"
            ),
          }));
          csv = json2csvParser.parse(csvData);
        ...
        res.attachment(
          `relatorio_${project.name.toLowerCase().replace(/\s+/g, "_")}.csv`
        );
        return res.send(csv);
      }
      ...
      if (format === "xlsx") {
        ...
        if (reportType === "hours") {
          worksheet.columns = [
            ...
            { header: "Período de Início", key: "inicio", width: 20 },
            { header: "Período de Fim", key: "fim", width: 20 },
            { header: "Total de Horas Gastas", key: "horas", width: 25 },
          ];
          worksheet.addRow({
            projeto: responsePayload.project,
            inicio: responsePayload.period.start,
            fim: responsePayload.period.end,
            horas: parseFloat(responsePayload.totalHours),
          });
        }

        if (reportType === "tickets") {
          worksheet.columns = [
            { header: "ID do Chamado", key: "id", width: 15 },
            { header: "Título", key: "titulo", width: 40 },
            { header: "Criado Por", key: "criadoPor", width: 25 },
            { header: "Status", key: "status", width: 15 },
            { header: "Prioridade", key: "prioridade", width: 15 },
            { header: "Data de Criação", key: "criadoEm", width: 20 },
          ];
          (responsePayload.tickets || []).forEach((t) => {
            worksheet.addRow({
              id: t.id,
              titulo: t.title,
              criadoPor: t.Client?.name || "N/D",
              status: t.status,
              prioridade: t.priority,
              criadoEm: new Date(t.createdAt).toLocaleDateString("pt-BR"),
            });
          });
        }
      ...
      return res
        .status(500)
        .json({ message: "Ocorreu um erro interno ao gerar o relatório." });
    }
  }
);
