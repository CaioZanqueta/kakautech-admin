...
const commentSchema = yup.object().shape({
  content: yup.string().required('O comentário não pode estar vazio.').min(3, 'O comentário é muito curto.'),
});
...
router.post("/portal/login", passport.authenticate("local-client", {
...
router.get("/portal/register", async (req, res) => {
  ...
  res.render("client/client-register", { error: null, projects, message: null });
});
...
router.post("/portal/register", async (req, res, next) => {
  ...
  const projects = await Project.findAll({ where: { status: 'active' } });
  try {
    await registerSchema.validate({ name, email, password, projectId }, { abortEarly: false });
    const existingClient = await Client.findOne({ where: { email } });
    if (existingClient) {
      return res.status(400).render("client/client-register", { error: "Este e-mail já está cadastrado...", projects, message: null });
    }
    await Client.create({ name, email, password, projectId, status: 'pending' });
    return res.render("client/client-register", { message: "Solicitação de cadastro enviada com sucesso!", projects, error: null });
  } catch (error) {
    if (error instanceof yup.ValidationError) {
      return res.status(400).render("client/client-register", { error: error.errors.join('. '), projects, message: null });
    }
    console.error("Erro no cadastro do cliente:", error);
    return res.status(500).render("client/client-register", { error: "Ocorreu um erro inesperado.", projects, message: null });
  }
});
...
router.get("/auth/google", passport.authenticate("google-client", { scope: ["profile", "email"] }));
router.get("/auth/google/callback", passport.authenticate("google-client", { failureRedirect: "/portal/login", failureMessage: true }), socialAuthCallback);
router.get("/auth/microsoft", passport.authenticate("microsoft-client"));
router.get("/auth/microsoft/callback", passport.authenticate("microsoft-client", { failureRedirect: "/portal/login", failureMessage: true }), socialAuthCallback);
...
router.get("/portal/download/ticket/:recordId", requireClientAuth, async (req, res) => {
  try {
    const { recordId } = req.params;
    const ticket = await Ticket.findByPk(recordId);
    if (!ticket || !ticket.path) { return res.status(404).send("Anexo não encontrado."); }
    if (ticket.clientId !== req.user.id) { return res.status(403).send("Acesso negado."); }
...
router.get("/portal/tickets", requireClientAuth, async (req, res) => {
  try {
    const tickets = await Ticket.findAll({ where: { clientId: req.user.id }, order: [['updatedAt', 'DESC']] });
...
router.post("/portal/tickets/:id/comments", requireClientAuth, async (req, res) => {
  const ticketId = req.params.id;
  try {
    const { content } = req.body;
    await commentSchema.validate({ content });

    const ticket = await Ticket.findOne({ where: { id: ticketId, clientId: req.user.id } });
    ...
    if (error instanceof yup.ValidationError) {
        return res.status(400).render(`portal/ticket-detail`, { user: req.user, ticket, error: error.message });
    }
...
router.post(
  ...
  async (req, res) => {
    try {
      ...
      const ticketData = {
        title, description, clientId: req.user.id,
        projectId: req.user.projectId, status: "open",
      };
      if (req.file) {
      ...
      try {
        ...
        const emailHtml = await ejs.renderFile(
          ...
          { 
            clientName: req.user.name, clientEmail: req.user.email,
            ticketTitle: newTicket.title, ticketDescription: newTicket.description,
      ...
      if (error instanceof yup.ValidationError) {
          return res.status(400).render("portal/new-ticket", { message: null, error: error.message, user: req.user });
      }
      console.error("Erro ao criar chamado:", error);
      res.status(500).render("portal/new-ticket", { message: null, error: `Erro: ${error.message}`, user: req.user });
    }
  }
);
